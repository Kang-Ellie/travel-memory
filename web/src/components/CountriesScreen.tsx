        ...
        >
          ...
          <div className="field">
            ...
            <input
              ...
              value={flightAirport}
              placeholder="예: KIX"
              onChange={(e) => setFlightAirport(e.target.value)}
            />
          </div>
          <div className="field">
            <label>직항/경유</label>
...
function CountryCard({
  ...
  const remove = async () => {
    if (
      !confirm(
        `'${country.name}' 국가를 삭제할까요? 소속된 도시도 함께 삭제돼요.`,
      )
    )
      return;
    ...
    <>
      ...
      {detailOpen && (
        ...
        >
          ...
          {(country.prepDocs || country.prepDocsUrl) && (
            <div
            ...
            >
              <span>📋 준비서류 : {country.prepDocs || "—"}</span>
              {country.prepDocsUrl && (
                <a
                  className="btn small primary"
                  href={country.prepDocsUrl}
                  target="_blank"
                  rel="noreferrer"
                >
                  🔗 온라인 신청
      ...
      {showAddCity && (
        ...
        >
          ...
          >
            ...
            <div style={{ marginTop: 12 }}>
              <button className="btn small primary" onClick={addCity}>
                ＋ 도시 추가
              </button>
            </div>
          </div>
