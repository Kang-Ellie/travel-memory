...
const mapPlace = (r: any) => ({
  ...
  grade: r.grade, stayType: r.stay_type, directions: r.directions, babyMenu: r.baby_menu,
...
export function registerRoutes(app: ExpressApp): void {
  ...
  app.post('/api/places', async (req, res) => {
    const {
      ...
      valetCompany, bookingChannel, grade, stayType, directions, babyMenu, recommend, tip,
    } = req.body as {
    ...
    await pool.query(
      `INSERT INTO places (id, name, address, category, lat, lng, memo, map_url, rating, pros, cons, country_id, city_id,
         ...
         valet_company, booking_channel, grade, stay_type, directions, baby_menu, recommend, tip)
  ...
  app.put('/api/places/:id', async (req, res) => {
    ...
    await pool.query(
      ...
      [name.trim(), (address ?? '').trim(), category, memo, mapUrl?.trim() || null, rating ?? null,
        ...
        directions?.trim() || null, babyMenu?.trim() || null, recommend ?? null, tip?.trim() || null,
        stayType?.trim() || null, req.params.id])
    res.json({ ok: true })
  })
